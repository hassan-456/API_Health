...
export const EndpointDetailModal: React.FC<EndpointDetailModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-md animate-in fade-in duration-200">
      <div className="glass-panel border border-slate-700 rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden shadow-2xl">
        {/* Header */}
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-xs text-slate-400 hover:text-indigo-400 font-mono mt-1 inline-flex items-center space-x-1"
              >
        ...
        <div className="p-6 overflow-y-auto space-y-6 text-sm">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="p-3 rounded-xl bg-slate-900/60 border border-slate-800 text-center">
              ...
              <div className="font-bold text-sm">
                {!endpoint.isActive ? (
                  ...
                  <span className="text-rose-400">Down / Outage</span>
                ) : endpoint.lastCheck.thresholdExceeded ? (
                  <span className="text-amber-400">Degraded SLA</span>
                ) : (
            ...
            <div className="p-3 rounded-xl bg-slate-900/60 border border-slate-800 text-center">
              <span className="text-[11px] text-slate-400 block mb-1">Latest Latency</span>
                  ...
                  <Tooltip
                    content={({ active, payload }) => {
                      if (active && payload && payload.length) {
                        ...
                        return (
                          <div className="glass-panel p-2 rounded text-xs border border-slate-700">
                            <span className="text-slate-400">{d.time}:</span>{' '}
                            <span className="font-mono font-bold text-white">{d.latency}ms</span>
                          </div>
                        );
                      }
                      return null;
                ...
                <tbody className="divide-y divide-slate-800/60 text-slate-300">
                  {isLoading ? (
                    ...
                    history.slice(0, 8).map((c) => (
                      <tr key={c._id} className="hover:bg-slate-900/60">
