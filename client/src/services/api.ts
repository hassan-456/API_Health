...
const LOCAL_STORAGE_INCIDENTS_KEY = 'pulsewatch_incidents';
...
function getStoredIncidents(): Incident[] {
  try {
    const data = localStorage.getItem(LOCAL_STORAGE_INCIDENTS_KEY);
    if (data) return JSON.parse(data);
  } catch {
    // ignore
  }

  // First run: replay the seeded history through the classifier so Demo
  // Mode starts with a realistic incident timeline, same as a live backend
  // that had been running for a while.
  const incidents = deriveInitialIncidents(getStoredEndpoints(), getStoredHistory());
...
export const api = {
  ...
  async getDashboard(): Promise<DashboardStats> {
    ...
    return computeDashboardStats(endpoints, history, incidents);
  },
  ...
  async deleteEndpoint(id: string): Promise<void> {
    ...
    const incidents = getStoredIncidents().filter((inc) => inc.apiId !== id);
    saveStoredIncidents(incidents);
  },
  ...
  async executeCheck(id: string): Promise<HealthCheck> {
    ...
    // Classify the outcome and open/update/resolve an incident, same as live backend
    const incidents = applyCheckToIncidents(getStoredIncidents(), endpoint, newCheck);
    saveStoredIncidents(incidents);
  ...
  async getIncidents(filters?: {
    ...
    try {
      ...
      const res = await axios.get('/api/incidents', { params, timeout: 1500 });
      if (res.data?.success && Array.isArray(res.data.data)) {
        backendHealthy = true;
        return res.data.data;
      }
    ...
    if (filters?.apiId) incidents = incidents.filter((i) => i.apiId === filters.apiId);
  ...
  resetDemoData() {
    ...
    localStorage.removeItem(LOCAL_STORAGE_INCIDENTS_KEY);
  },
};
